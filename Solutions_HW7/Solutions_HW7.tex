...
% We let $\frac{d}{dt}\mathcal{E}_t \leq 0$ and we get 
% \[
% \frac{1}{2}||x^*-X_t||^2 \leq e^{\frac{2mL}{m+L}t}\mathcal{E}_0
% \]
% and an $O(e^{\frac{2mL}{m+L}t})$ convergence rate. In addition, smoothness can be used to conclude a tighter bound on the convergence of the optimal gap using this tighter bound,
% \[
% f(X_t)-f(x^*) \leq \frac{L}{2}||x^*-X_t||^2 \leq e^{-\frac{2mL}{m+L}t}\frac{L}{2}||x^*-X_0||^2.
% \]
\\
...
% In particular, if we choose $\delta=\frac{2}{m+L}$, then we have $f(x_k)-f(x^*)\leq O(\frac{L}{2}e^{-\frac{4mLk}{(m+L)^2}})$.
\end{proof}
